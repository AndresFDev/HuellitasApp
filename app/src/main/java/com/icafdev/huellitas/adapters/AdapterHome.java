...
public class AdapterHome extends FirestoreRecyclerAdapter<Entry, AdapterHome.ViewHolder> {

    private Fragment context;

    public AdapterHome(FirestoreRecyclerOptions<Entry> options, Fragment context) {
        super(options);
    ...
    protected void onBindViewHolder(@NonNull ViewHolder holder, int position, @NonNull Entry entry) {
        ...
        holder.gen = entry.getGen();
        holder.tv_raza.setText(entry.getRaza());
        holder.tv_date.setText(entry.getDate());
        holder.tv_city.setText(entry.getCity());
        holder.tv_description.setText(entry.getDescription());

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            holder.tv_description.setJustificationMode(LineBreaker.JUSTIFICATION_MODE_INTER_WORD);
        }
        ...
        if (holder.tv_description.getText().toString().length() < 200) {
            holder.tv_suspensive.setVisibility(View.GONE);
        }else {
            holder.tv_suspensive.setVisibility(View.VISIBLE);
        }

        if (holder.type != null) {
            if (holder.gen.equals("Macho")) {
                holder.iv_gen.setImageResource(R.drawable.ic_male);
            } else {
                holder.iv_gen.setImageResource(R.drawable.ic_female);
            }
        }
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        private TextView tv_status, tv_name, tv_pet_name, tv_raza, tv_date, tv_city, tv_description, tv_suspensive;
        private ImageView iv_photo, iv_type, iv_gen;
        ...
        public ViewHolder(@NonNull View itemView){
            ...
            iv_gen = itemView.findViewById(R.id.iv_gen);
            tv_raza = itemView.findViewById(R.id.tv_raza);
            tv_date = itemView.findViewById(R.id.tv_date);
            tv_city = itemView.findViewById(R.id.tv_city);
            tv_description = itemView.findViewById(R.id.tv_description);
            tv_suspensive = itemView.findViewById(R.id.tv_suspensive);
